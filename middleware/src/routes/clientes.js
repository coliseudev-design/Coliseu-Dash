    ...
    try {
        ...
        const { rows } = await db.query(`
            SELECT 
                ...
                (SELECT COALESCE(SUM(v.valor_total), 0) FROM dash_vendas v WHERE v.cliente_id_firebird = c.id_firebird AND v.tenant_id = c.tenant_id AND v.status = 'FINALIZADO') AS total_gasto
