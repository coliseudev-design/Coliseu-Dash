...
router.get('/resumo-mes', async (req, res, next) => {
    try {
        ...
        const { rows } = await db.query(`
            SELECT
                SUM(CASE WHEN TRIM(f.tipo) = 'RECEBER' AND TRIM(f.status_pagamento) = 'PAGO' AND f.data_pagamento >= $2 AND f.data_pagamento <= $3 THEN f.valor_pago ELSE 0 END) AS total_recebido,
                ...
                SUM(CASE WHEN TRIM(f.tipo) = 'PAGAR' AND TRIM(f.status_pagamento) = 'PAGO' AND f.data_pagamento >= $2 AND f.data_pagamento <= $3 THEN f.valor_pago ELSE 0 END) AS total_pago,
