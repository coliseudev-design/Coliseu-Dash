...
router.get('/overview', async (req, res, next) => {
    try {
        ...
        const [
            ...
            db.query(`SELECT COALESCE(SUM(valor - valor_pago),0) AS v FROM dash_financeiro WHERE tenant_id = $1 AND COALESCE(data_vencimento, data_emissao, NOW()) >= $2 AND COALESCE(data_vencimento, data_emissao, NOW()) <= $3 AND tipo = 'RECEBER' AND TRIM(status_pagamento) = 'ABERTO'`, [tenantId, finRange.start, finRange.end]),
            db.query(`SELECT COALESCE(SUM((CASE WHEN valor_pago = 0 THEN valor ELSE valor_pago END)),0) AS v FROM dash_financeiro WHERE tenant_id = $1 AND COALESCE(data_pagamento, data_vencimento, NOW()) >= $2 AND COALESCE(data_pagamento, data_vencimento, NOW()) <= $3 AND tipo = 'RECEBER' AND TRIM(status_pagamento) = 'PAGO'`, [tenantId, finRange.start, finRange.end]),
            db.query(`SELECT COALESCE(SUM(valor - valor_pago),0) AS v FROM dash_financeiro WHERE tenant_id = $1 AND COALESCE(data_vencimento, data_emissao, NOW()) >= $2 AND COALESCE(data_vencimento, data_emissao, NOW()) <= $3 AND tipo = 'PAGAR' AND TRIM(status_pagamento) = 'ABERTO'`, [tenantId, finRange.start, finRange.end]),
            db.query(`SELECT COALESCE(SUM((CASE WHEN valor_pago = 0 THEN valor ELSE valor_pago END)),0) AS v FROM dash_financeiro WHERE tenant_id = $1 AND COALESCE(data_pagamento, data_vencimento, NOW()) >= $2 AND COALESCE(data_pagamento, data_vencimento, NOW()) <= $3 AND tipo = 'PAGAR' AND TRIM(status_pagamento) = 'PAGO'`, [tenantId, finRange.start, finRange.end]),
...
router.get('/kpis', async (req, res, next) => {
    try {
        ...
        const { rows: f } = await db.query(`
            SELECT 
                COALESCE(SUM(CASE WHEN tipo = 'RECEBER' AND TRIM(status_pagamento) = 'ABERTO' AND COALESCE(data_vencimento, data_emissao, NOW()) >= $2 AND COALESCE(data_vencimento, data_emissao, NOW()) <= $3 THEN valor - (CASE WHEN valor_pago = 0 THEN 0 ELSE valor_pago END) ELSE 0 END), 0) AS a_receber,
                COALESCE(SUM(CASE WHEN tipo = 'RECEBER' AND TRIM(status_pagamento) = 'PAGO' AND COALESCE(data_pagamento, data_vencimento, NOW()) >= $2 AND COALESCE(data_pagamento, data_vencimento, NOW()) <= $3 THEN (CASE WHEN valor_pago = 0 THEN valor ELSE valor_pago END) ELSE 0 END), 0) AS recebido,
                COALESCE(SUM(CASE WHEN tipo = 'PAGAR' AND TRIM(status_pagamento) = 'ABERTO' AND COALESCE(data_vencimento, data_emissao, NOW()) >= $2 AND COALESCE(data_vencimento, data_emissao, NOW()) <= $3 THEN valor - (CASE WHEN valor_pago = 0 THEN 0 ELSE valor_pago END) ELSE 0 END), 0) AS a_pagar
