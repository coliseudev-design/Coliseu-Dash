...
print("=== FIND RODOLFO PAULO SCHLATTER ===")
sql = f"""
SELECT id_firebird, nome 
FROM dash_clientes 
WHERE tenant_id = '{tenant_id}' AND nome LIKE '%SCHLATTER%'
"""
print(run_query("coliseu_dashboard", sql))

print("=== ALL SALES FOR RODOLFO IN JUNE 2026 ===")
sql = f"""
SELECT 
    id_firebird, 
    valor_total, 
...
WHERE tenant_id = '{tenant_id}' 
  AND cliente_id_firebird = (SELECT id_firebird FROM dash_clientes WHERE tenant_id = '{tenant_id}' AND nome LIKE '%SCHLATTER%' LIMIT 1)
