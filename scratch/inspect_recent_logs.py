...
if frontend_container:
    print(f"\n=== Frontend Container Logs (Access): {frontend_container} ===")
    run_cmd(f"docker exec {frontend_container} tail -n 20 /var/log/nginx/access.log")
    print(f"\n=== Frontend Container Logs (Error): {frontend_container} ===")
    run_cmd(f"docker exec {frontend_container} tail -n 20 /var/log/nginx/error.log")

if middleware_container:
    print(f"\n=== Middleware Logs: {middleware_container} ===")
