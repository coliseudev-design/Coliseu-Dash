...
run_query(
    "coliseu_dashboard",
    "SEARCH BY ID_FIREBIRD OR NUMERO_PEDIDO",
    "SELECT id_firebird, numero_pedido, tenant_id, data_venda, valor_total, status FROM dash_vendas WHERE id_firebird IN (18714, 18737) OR numero_pedido IN ('6776', '11374');"
)
...
# Also let's check general row counts and tenant list again just to be 100% sure we didn't miss something
run_query(
    "coliseu_dashboard",
    "ROW COUNT AND TENANTS",
