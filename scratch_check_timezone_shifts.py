...
try:
    ...
    # Query early morning sales (00:00 to 03:00 UTC) in June 2026
    sql = (
        f"SELECT id_firebird, numero_pedido, data_venda, "
        f"       (valor_total - COALESCE(valor_desconto, 0)) as neto, "
        f"       especie, status "
        f"FROM dash_vendas "
        ...
        f"  AND EXTRACT(HOUR FROM COALESCE(data_vencimento, data_venda)) < 3 "
    ...
    print("=== SALES IN EARLY HOURS (00:00 - 03:00 UTC) IN JUNE ===")
