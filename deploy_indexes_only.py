...
# SQL to perform index creation only
sql_content = """
-- 1. Create Indexes on dash_vendas
...
-- 2. Create Indexes on dash_financeiro
...
-- 3. Create Indexes on dash_devolucoes
...
with sftp.open('/tmp/indexes_only.sql', 'w') as f:
    f.write(sql_content)
sftp.close()

# Copy file into docker container
client.exec_command(f"docker cp /tmp/indexes_only.sql {db_container}:/tmp/indexes_only.sql")

# Execute SQL script with psql
print("🚀 Creating indexes on production database container...")
stdin, stdout, stderr = client.exec_command(f"docker exec {db_container} psql -U coliseu_admin -d coliseu_dashboard -f /tmp/indexes_only.sql")
print("=== PSQL OUTPUT ===")
print(stdout.read().decode('utf-8'))
print(stderr.read().decode('utf-8'))

# Clean up
client.exec_command("rm /tmp/indexes_only.sql")
client.exec_command(f"docker exec {db_container} rm /tmp/indexes_only.sql")

client.close()
print("✅ Index creation script executed completely!")
