...
try:
    ...
    # Query sum of negative sales in June 2026 grouped by day
    sql = (
        f"SELECT COALESCE(data_vencimento::date, data_venda::date) as dia, "
        f"       COUNT(*), "
        f"       SUM(valor_total - COALESCE(valor_desconto, 0)) as total_neg "
        f"FROM dash_vendas "
    ...
    print("=== DAILY NEGATIVE SALES IN JUNE ===")
